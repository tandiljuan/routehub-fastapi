...
CREATE TABLE IF NOT EXISTS milestone (
  ...
  name TEXT NOT NULL,
  location TEXT,
  category TEXT CHECK( category IN ( 'DEPOT', 'DISTRIBUTION_CENTER', 'LOADING_POINT', 'WAREHOUSE', 'TRANSFER_POINT' ) ) NOT NULL,
  FOREIGN KEY(company_id) REFERENCES company(id) ON DELETE CASCADE
);
CREATE TABLE IF NOT EXISTS delivery (
  id INTEGER PRIMARY KEY,
  company_id INTEGER NOT NULL,
  method TEXT CHECK( method IN ( 'DELIVERY', 'PICKUP', 'SIGNATURE' ) ) NOT NULL,
  ...
  height INTEGER,
  depth INTEGER,
  length_unit TEXT CHECK( length_unit IN ( 'CENTIMETER', 'METER', 'INCH', 'FEET' ) ) NOT NULL,
  volume INTEGER,
  volume_unit TEXT CHECK( volume_unit IN ( 'CUBIC_CENTIMETER', 'CUBIC_METER', 'LITER', 'CUBIC_INCH', 'CUBIC_FEET', 'GALLON' ) ) NOT NULL,
  weight INTEGER,
  weight_unit TEXT CHECK( weight_unit IN ( 'GRAMS', 'KILOGRAMS', 'OUNCES', 'POUNDS' ) ) NOT NULL,
  packaging TEXT CHECK( packaging IN ( 'BOX', 'ENVELOPE', 'BAG', 'CARTON', 'PALLET', 'SHRINK_WRAP' ) ) NOT NULL,
  handling TEXT CHECK( handling IN ( 'FRAGILE', 'PERISHABLE', 'HAZARDOUS', 'REFRIGERATED' ) ) NOT NULL,
  value_cents INTEGER,
  value_currency TEXT,
  extra TEXT,
  FOREIGN KEY(company_id) REFERENCES company(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS delivery_lot (
  ...
  route_length_unit TEXT CHECK( route_length_unit IN ( 'CENTIMETER', 'METER', 'INCH', 'FEET' ) ) NOT NULL,
  route_time_max INTEGER,
  route_time_unit TEXT CHECK( route_time_unit IN ( 'SECOND', 'MINUTE', 'HOUR' ) ) NOT NULL,
  FOREIGN KEY(company_id) REFERENCES company(id) ON DELETE CASCADE,
  FOREIGN KEY(milestone_id) REFERENCES milestone(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS _94be50f2d82842b08e0c5ed26321af08 ON fleet_vehicle (fleet_id, vehicle_id);
CREATE INDEX IF NOT EXISTS _a1ae650c77e14e7e900731e4d9e6eb61 ON driver_vehicle (driver_id, vehicle_id);
CREATE INDEX IF NOT EXISTS _472616e1f3c24ef0a0d1770995b14dbe ON delivery_lot_delivery (delivery_lot_id, delivery_id);
CREATE INDEX IF NOT EXISTS _7f7aa4b0f4594761a3fc6a1b046faa3c ON delivery_lot_driver (delivery_lot_id, driver_id);
CREATE INDEX IF NOT EXISTS _8e99025852db48f1aa473cc6a807181e ON delivery_path_delivery (delivery_path_id, delivery_id);
